  ...
  for (const modelName of GEMINI_MODELS) {
    ...
    try {
      ...
      if (process.env.DEBUG_LLM) {
        console.error(`Gemini API error on model ${modelName}:`, err instanceof Error ? err.message : err);
      }
    }
  }
...
async function explainWithGroq(
  ...
  for (const modelName of GROQ_MODELS) {
    ...
    try {
      ...
      clearTimeout(timeoutId);
      if (process.env.DEBUG_LLM) {
        console.error(`Groq API error on model ${modelName}:`, err instanceof Error ? err.message : err);
      }
    }
  }
